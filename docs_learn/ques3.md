...
pos_sum = (-15) + (-5) + (+5) = -15
black_count = 3
Gray_Line_Pos_mm = -15 / 3 = -5 mm  → 黑线在中心偏左 5mm
```

如果只有传感器 3 看到：

```
Gray_Line_Pos_mm = -5 / 1 = -5 mm  → 同样结果，但置信度低
```
...
| 条件 | 含义 | 速度 | 
| ---- | ---- | ---- |
| 黑线在中心 ±10mm 内 | 直线/微偏 | 300 mm/s |
| 黑线偏离中心 10~22mm | 缓弯 | 240 mm/s |
| 黑线偏离超过 22mm | 急弯 | 185 mm/s |
| ≥5 个传感器同时看到黑线 | 可能是直角弯/交叉 | 185 mm/s |
| 十字/全黑 (≥7个) | 直冲 | 310 mm/s（特殊逻辑，见 Q4） |
...
过十字路口时，8 个传感器全部看到黑线（`black_count >= 7`），无法判断线在哪个方向。如果正常巡线算法去算，会得到一个中心附近的位置，然后车会**直走冲过十字**——这正好是我们想要的，但需要防止误触发。
