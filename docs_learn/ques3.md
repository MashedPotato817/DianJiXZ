...
核心思想：当前车辆的传感器在左右轮中心中点前方 `GRAY_SENSOR_FORWARD_MM = 134mm` 处，测到的偏差 $y$ 可以用来估算路径的曲率。
...
偏差越大 → 曲率越大 → 转角越大。当前前视距离 134mm 决定了："提前"134mm 看路。前视距离缩短时，同一偏差计算出的曲率会增大，因此须重新实车标定转向增益。
...
| `GRAY_SENSOR_FORWARD_MM` | 134 | 前视距离，决定预判距离 |
