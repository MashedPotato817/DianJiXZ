...
当前 `project/mspm0` 已有中心死区（`GRAY_CENTER_DEADBAND_MM`），应先实测其是否覆盖真实居中读数、并记录左右目标/实际速度/PWM。**不应**在没有这份证据前迁移 `demo2` 的 PD、IMU 航向、丢线斜坡、路口保持、起步斜坡或电机卡滞保护。
...
| 中心处理 | 中间单/双点与小误差同速 | `P` 减偏移后，`±7 mm` 置零 | 目标一致；当前先验证死区中心是否正确。 |
...
`demo2` 的代码意图明确：中心单点/双点与近中心误差均清零并输出相同左右目标，避免中间两路切换造成低频摆动。当前项目已经通过 `GRAY_CENTER_DEADBAND_MM` 实现同一原则。
