...
会话编号由脚本自动递增（序号存于 `LOG_FOLDER_PATH/.session_counter`），每次上电不重复。前缀在 `ball_detect_config.py` 设置：

```python
LOG_SESSION_PREFIX = "20260731"
```

K230文件名固定为 `<前缀>_<序号>_K230.txt`，电脑串口助手用启动时终端打印的 `LOG SESSION` 编号对应保存为 `<同一编号>_M0.txt`。计数器丢失时按目录内已有日志最大序号兜底递增，不覆盖旧日志。
...
3. `BALL_ROI` 初始为 `y=60…109 px`。运行时按 KEY2（IO0）向上、KEY1（IO35）向下微调，每次 `5 px`；调整仅在本次运行中生效。
