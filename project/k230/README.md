...
- **相机、摆杆和小球运动平面已机械固定**。已用静态 `-50/0/+50 mm` 三点数据完成临时预览标定；`CALIBRATION_READY` 仍为 `False`。临时 `x_mm` 可用于受限联调，不能用于正式控制/精度结论。
...
K230 板载时钟未校准时会显示2018年的日期，不能用它与电脑端M0日志自动配对。每次联调前在 `ball_detect_config.py` 设置共享测试编号：
...
```text
/data/ball_detect_preview/20260731_000007_K230.txt
```

电脑串口助手将同一次M0遥测保存为：

```text
20260731_000007_M0.txt
```

K230启动终端和TXT都会输出 `LOG SESSION: 20260731_000007`，可在采集前复核。日志以追加模式打开，因此每次新实验必须先递增编号，不能重复使用已经存在的编号。
