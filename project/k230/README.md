...
- **相机和摆杆尚未固定**，因此视觉标定暂不进行。`IMAGE_CENTER_X`、`MM_PER_PIXEL` 与 `MAX_POSITION_MM` 仅为待标定初值，当前 `x_mm` 不能作为真实物理距离，也不能用于舵机控制。
- 当前先完成画面预览和 UART 帧收发；待机械固定后，再补做中心、比例、方向与全行程误差标定。
...
| 缓慢向右移动 | `x_mm` 连续增大且为正。 |
| 缓慢向左移动 | `x_mm` 连续减小且为负。 |
| 遮住或移走小球 | 约 `LOST_FRAMES` 帧后显示 `BALL LOST`。 |
...
> 当前不执行本节：相机和摆杆尚未固定，无法建立可靠的像素—毫米映射。此期间可观察 `BALL OK/LOST` 和像素运动趋势，但不得据 `x_mm` 判断实际位置或驱动舵机。
