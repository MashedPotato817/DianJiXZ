...
- K230 基础数据帧：`$K230,BALL,<x_mm>,<valid>,<seq>#`；视觉脚本可追加边缘字段：`$K230,BALL,<x_mm>,<valid>,<seq>,<edge>#`。
- MSPM0 心跳：每 500 ms 发送 `$MSPM0,HELLO#\r\n`。
- 主控超时：连续 100 ms 未收到完整合法帧时，将位置 `valid` 置为 `0`。

`x_mm` 为相对摆杆中心的毫米坐标，右正左负；`valid` 只能为 `0` 或 `1`；`seq` 为递增无符号序号。可选 `edge` 为 `-1/0/+1`：仅在球最后可靠位置靠近图像左/右边缘、随后连续丢失时置为 `-1/+1`，普通漏检为 `0`。

## 最小实机步骤

1. 向 K230 上传并运行 `project/k230/k230_uart_ball_test.py`。
2. 烧录当前 MSPM0 固件，不接入舵机或 `Ball_Control_Step()`。只保留 K230 IO40 到 MSPM0 PB7 和共地；回传线异常时可暂不连接 IO41。
