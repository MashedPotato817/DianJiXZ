...
void Gray_Mode(void)
{
    ...
    /* 中心连续软化：|pos|<GRAY_SOFTEN_MM 时误差被三次函数压小，中心处梯度趋零，
       消除硬死区边界的转向阶跃。外侧渐进恢复线性，弯道能力不损失。 */
    if (Gray_Line_Pos_mm > -GRAY_SOFTEN_MM && Gray_Line_Pos_mm < GRAY_SOFTEN_MM) {
        float r = Gray_Line_Pos_mm / GRAY_SOFTEN_MM;   /* -1..+1 归一化 */
        Gray_Line_Pos_mm = Gray_Line_Pos_mm * r * r;   /* 三次软化：x*x²，中心平坦外侧渐强 */
    }
    Move_X = GRAY_BASE_SPEED_MM_S / 1000.0f;
