...
1. 舵机：PA8/TIMA0，50 Hz；机械允许5~175°。当前固定上电平衡基准为 100.8°（PWM 1660 us）；自动标定或 UART 调参可在本次上电期间修改 `trim`，但 Flash 旧值不会在下次上电时覆盖该基准。
...
截至 2026-07-31 21:20，单帧运动确认、脱困后卸力、中心非线性阻尼与受控过零 `PASS` 状态已完成实机验证（均在硬件修缮前）。硬件修缮后旧日志中的平衡值已失效。2026-08-01 当前机构平衡基准更新为 **PWM 1660 us（=100.8°）**；源码通过 `BALL_CAL_LOAD_ENABLE=0` 禁止 Flash 旧标定在上电时覆盖该值，trim 在线学习也保持关闭。该基准修改需要重新构建、烧录和实机静止验证，不能沿用旧硬件日志作为验证结论。详见 [`../preview/球杆平衡闭环算法分析与改进方案_20260730.md`](../preview/球杆平衡闭环算法分析与改进方案_20260730.md)。
