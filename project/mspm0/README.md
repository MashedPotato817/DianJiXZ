...
截至 2026-07-31 21:20，单帧运动确认、脱困后卸力、中心非线性阻尼与受控过零 `PASS` 状态已完成实机验证（均在硬件修缮前）。硬件修缮期间舵机被重新安装，修缮后实测（`000009`/`000010`）均未能进入居中静止状态，且位置持续偏向一侧；据此新增**自动扫掠标定**（`Control/ball_calibrate.*`）并将**标定结果持久化到 Flash**（`Hardware/calib_store.*`，存于 `0x0001F800`，上电自动读取，Flash 无有效数据时回退默认 122.4°）。`000012` 实测确认链路中断优先级修复生效（`parse_err`/`crc_err`/`rx_hw_overrun` 全程为 0）；用户确认 **PWM 1780 us（=122.4°）即当前机构平衡值**，`000012` 中球加速冲出画面是 trim 在线学习把 trim 漂离 1780 所致，因此 trim 学习已大幅收窄、接近关闭（单步≤0.03°），且自动扫掠标定已以编译期开关 `BALL_CAL_ENABLE=0` 暂时关闭（代码保留，换硬件标定时改为 1 重新编译）。以上改动均已通过 Keil 构建（`0 Error(s), 0 Warning(s)`），均尚待硬件验证。详见 [`../docs/球杆平衡闭环算法分析与改进方案_20260730.md`](../docs/球杆平衡闭环算法分析与改进方案_20260730.md) 第14~22节。

> 最后更新：2026-07-31 21:20
