...
截至 2026-07-31 21:20，单帧运动确认、脱困后卸力、中心非线性阻尼与受控过零 `PASS` 状态已完成实机验证（均在硬件修缮前）。硬件修缮后旧日志中的平衡值已失效。2026-08-01 新日志测得无漂移值 **PWM 1701 us（=108.1°）**，已作为无有效 Flash 数据时的默认基准；该次球停在约 +33 mm，因此“无漂移”不等于“已经居中”。当前程序在 RESET 后明确设置目标 0 cm；START（PA18）短按执行 `0→+5→-5 cm`，长按执行自动标定。标定结果保存到链接器保留的最后 1KB Flash 扇区 `0x0001FC00`，保存后读回验证，RESET 后优先加载。位置精度、动作顺序和掉电保持仍需烧录后实机验收。详见 [`../preview/球杆平衡闭环算法分析与改进方案_20260730.md`](../preview/球杆平衡闭环算法分析与改进方案_20260730.md)。
