...
float Velcity_Kp=0.4,  Velcity_Ki=1.0,  Velcity_Kd; //相关速度PID参数
...
{  
	...
	Last_biasA=Bias;	
	limit_PWM(ControlVelocityA,-7000,7000);
	return (int)ControlVelocityA; //返回速度控制值
}
...
{  
	...
	ControlVelocityB+=Velcity_Kp*(Bias-Last_biasB)+Velcity_Ki*Bias;  //增量式PI控制器
                                                               //Velcity_Kp*(Bias-Last_bias) 作用为限制加速度
	                                                            //Velcity_Ki*Bias             速度控制值由Bias不断积分得到 偏差越大加速度越大
	Last_biasB=Bias;	
	limit_PWM(ControlVelocityB,-7000,7000);
	return (int)ControlVelocityB; //返回速度控制值
}
