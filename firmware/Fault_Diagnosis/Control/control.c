...
volatile uint8_t Fault_Diag_Mode = FAULT_DIAG_STOP;
volatile uint16_t Fault_Diag_PWM_Left;
volatile uint16_t Fault_Diag_PWM_Right;
...
static void Fault_Diag_Apply(void)
{
    int left_pwm = 0;
    int right_pwm = 0;

    switch (Fault_Diag_Mode) {
    case FAULT_DIAG_LEFT:
        left_pwm = -FAULT_DIAG_PWM;
        break;
    case FAULT_DIAG_RIGHT:
        right_pwm = -FAULT_DIAG_PWM;
        break;
    case FAULT_DIAG_BOTH:
        left_pwm = -FAULT_DIAG_PWM;
        right_pwm = -FAULT_DIAG_PWM;
        break;
    case FAULT_DIAG_GRAY_GATE:
        if (Gray_Data[FAULT_DIAG_GRAY_CHANNEL]) {
            left_pwm = -FAULT_DIAG_PWM;
            right_pwm = -FAULT_DIAG_PWM;
        }
        break;
    ...
    Fault_Diag_PWM_Left = (uint16_t)myabs(left_pwm);
    Fault_Diag_PWM_Right = (uint16_t)myabs(right_pwm);
...
void Key(void)
{
	...
	if (tmp == USEKEY_single_click) {
		Fault_Diag_Mode++;
		if (Fault_Diag_Mode > FAULT_DIAG_GRAY_GATE) {
			Fault_Diag_Mode = FAULT_DIAG_STOP;
		}
	} else if ((tmp == USEKEY_double_click) || (tmp == USEKEY_long_click)) {
		Fault_Diag_Mode = FAULT_DIAG_STOP;
	}

	Flag_Stop = (Fault_Diag_Mode == FAULT_DIAG_STOP) ? 1 : 0;
}
