...
static void Fault_Diag_ShowVoltage(uint8_t x, uint8_t y, float voltage)
{
    uint16_t tenths = (voltage > 0.0f) ? (uint16_t)(voltage * 10.0f) : 0U;

    OLED_ShowNumber(x, y, tenths / 10U, 2, 12);
    OLED_ShowString(x + 12U, y, ".");
    OLED_ShowNumber(x + 18U, y, tenths % 10U, 1, 12);
    OLED_ShowString(x + 24U, y, "V");
}
/**************************************************************************
...
void oled_show(void)
{
     u8 i;
     int gray_pos_show;
     static float voltage_min;
     memset(OLED_GRAM,0, 128*8*sizeof(u8)); //GRAM清零但不立即刷新，防止花屏

     if ((Voltage > 1.0f) && ((voltage_min == 0.0f) || (Voltage < voltage_min))) {
         voltage_min = Voltage;
     }

     OLED_ShowString(0, 0, "FAULT");
     Fault_Diag_ShowVoltage(42, 0, Voltage);
     OLED_ShowString(0, 10, "M:");
     if (Fault_Diag_Mode == FAULT_DIAG_STOP) {
         OLED_ShowString(18, 10, "STOP");
     } else if (Fault_Diag_Mode == FAULT_DIAG_LEFT) {
         OLED_ShowString(18, 10, "LEFT");
     } else if (Fault_Diag_Mode == FAULT_DIAG_RIGHT) {
         OLED_ShowString(18, 10, "RIGHT");
     } else if (Fault_Diag_Mode == FAULT_DIAG_BOTH) {
         OLED_ShowString(18, 10, "BOTH");
     } else {
         OLED_ShowString(18, 10, "GRAY");
     }
     OLED_ShowString(0, 20, "G:");
     for (i = 0; i < 8; i++) {
         OLED_ShowString(16 + i * 10, 20, Gray_Raw[i] ? "1" : "0");
     }
     OLED_ShowString(0, 30, "L:");
     OLED_ShowNumber(18, 30, Fault_Diag_PWM_Left, 4, 12);
     OLED_ShowString(64, 30, "R:");
     OLED_ShowNumber(82, 30, Fault_Diag_PWM_Right, 4, 12);
     OLED_ShowString(0, 40, "MIN:");
     Fault_Diag_ShowVoltage(30, 40, voltage_min);
     OLED_ShowString(0, 50, "K1:N K2:S");
     OLED_Refresh_Gram();
     return;
        //=============第一行显示小车模式=======================//
